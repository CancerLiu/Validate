...
/**
 * Validator的核心类。继承拦截器相关类。通过Spring框架提供的面向切面方式来进入这里的逻辑
 */
public class ValidatorAdvisor extends HandlerInterceptorAdapter {
    ...
    public void validateHandler(ProceedingJoinPoint point) throws Throwable {
        ...
        logger.info(JsonMapper.buildNonNullMapper().toJson(objs));
        ...
        logger.info(JsonMapper.buildNonNullMapper().toJson(params));

        //最终判定，如果有错误将错误给前端
        ValidatorResult validatorResult = validatorHandler(params);
        logger.info(JsonMapper.buildNonNullMapper().toJson(validatorResult));
